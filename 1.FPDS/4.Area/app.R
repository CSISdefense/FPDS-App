...
# read in data            
FullData <- read.csv("FPDS_Area_data.csv")
...
plotsettings <- reactive({
  p <- ggplot(data = dataset(),
    ...
    #coll: Added title 
    ggtitle("Contract Obligations by Area") + 
...
plotsettings2 <- reactive({
  ...
  p <- ggplot(data = dataset(),
    ...
    facet_wrap(~ Category, nrow = 1, scales="free_x", drop = TRUE) + 
...
output$hover_info <- renderUI({
  ...
  switch(
    ...
    "Line" = {
      point <- nearPoints(dataset(), hover, xvar = "FY", yvar = "Percent",
                      threshold = (360 / (input$Yr[2] - input$Yr[1])) + 10,
  ...
  hov_amount <- dataset() %>%
    filter(FY == year & Category == point$Category) %>%
    .$Amount %>%
    unlist
