...
server <- function(input, output, session){
  ...
  DIIGfill <- scale_fill_manual(values = colorset, name = NULL)
  ...
  xbreaks <- rev(seq(
    ...
    p <- ggplot(dataset(),
      ...
      # theme(strip.text.x = element_text(family = "Arial", size = 16, color = "#554449")) + 
      ...
      # theme(axis.text.x = element_text(size = 14, color="#554449", margin=margin(-15,0,0,0))) +
      ...
      scale_x_continuous(
      breaks = xbreaks,
      labels = substring(as.character(xbreaks), 3, 4)) +
...
output$hover_info <- renderUI({
  ...
  hov_amount <- dataset() %>%
    filter(FY == year & Customer == point$Customer) %>%
    .$Amount %>%
    unlist
  ...
  style <- paste0(
   ...
   wellPanel(
    ...
    p(HTML(paste0("<b> Fiscal Year: </b>", round(hover$x) , "<br/>",
                  "<b> Component: </b>", point$Customer, "<br/>",
                  "<b> Amount: </b> $",
