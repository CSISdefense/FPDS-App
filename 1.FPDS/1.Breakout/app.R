...
server <- function(input, output, session){
  ...
  # read in data
  FullData <- read_csv("FPDS_Breakouts_data.csv")
  ...
  FullData$Breakout <- factor(FullData$Breakout, levels = c(
  ...
  nameset <-
    c(
      ...
      "MED" = "Medical ",
      "PAMS" = "Professional, Admin & Management ",

      # R&D titles
      "Basic Research (6.1)" = "Basic Research (6.1) ",
      "Applied Research (6.2)" = "Applied Research (6.2) ",
      "Adv. Technology Dev. (6.3)" = "Adv. Technology Dev. (6.3) ",
      "Adv. Component Dev. & Prototypes (6.4)" = "Adv. Component Dev. & Prototypes (6.4) ",
      "System Dev. & Demonstration (6.5)" = "System Dev. & Demonstration (6.5) ",
      "Operational Systems Dev. (6.7)" = "Operational Systems Dev. (6.7) "
    )
  ...
  plotsettings <- function(){
    ...
    p <- ggplot(data = plotdata,
      ...
      #           color = 'white', fontface = 'bold', family = 'Open Sans') +
      ...
      theme(axis.text.x = element_text(
        size = 15, family = "Open Sans", vjust=7, margin=margin(-10,0,0,0))) +
      theme(axis.text.y = element_text(
        size = 15, family = "Open Sans", color ="#554449", margin=margin(0,5,0,0))) +
      theme(axis.title.x = element_text(
        size = 16, face = "bold", color = "#554449", family = "Open Sans",
  ...
  clickplotsettings <- function(){
    ...
    p <- ggplot(data = clickplotdata, aes(x = FY, y = Billion)) +
      ...
      theme(plot.title = element_text(
        family = "Open Sans", color = "#554449", size = 26, face="bold", margin=margin(20,0,20,0))) +
      theme(axis.text.x = element_text(
        size = 15, family = "Open Sans", vjust=7, margin=margin(5,0,0,0))) +
      theme(axis.text.y = element_text(
        size = 15, family = "Open Sans", color ="#554449", margin=margin(0,5,0,0))) +
 ...
 output$CSVDownloadBtn <- downloadHandler(
  filename = paste('CSIS-Contract-Obligations-Breakout-', Sys.Date(),'.csv', sep=''),
  content = function(file) {
    writedata <- dataset()
    writedata <- select(writedata, -pos, -sumBillion)
  ...
  output$hover_info <- renderUI({
    ...
    wellPanel(
      style = style,
      p(HTML(paste0("<b>", cat_shown, "<br/>", "</b>", 
                    "<div align = 'left'>", 
                    "<b> Year: </b>", round(hover$x), "<br/>",
