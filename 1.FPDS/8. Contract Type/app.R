...
# FullData <- read.csv("FPDS_ContractType_data.csv")
# 
# FY <- levels(FullData$FY)
# FY <- c(FY)
# 
# Customer <- levels(FullData$Customer)
# Customer <- c(Customer) 
# 
# Category <- levels(FullData$Category) 
# Category <- c(Category)
# 
# Contract.Type <- levels(FullData$Contract.Type)
...
dataset <- reactive({
  ...
  # reorder by the final year
  shown$Contract.Type <- fct_reorder(
      shown$Contract.Type,
...
plotsettings <- reactive({
  p <- ggplot(data = dataset(),
    ...
    #coll: Added title 
    ggtitle("Contract Obligations by Contract Type") + 
...
plotsettings2 <- reactive({
  ...
  p <- ggplot(data = dataset(),
    ...
    facet_wrap(~ Contract.Type, nrow = 2, scales="free_x", drop = TRUE) + 
...
output$CSVDownloadBtn <- downloadHandler(
  filename = paste('CSIS-Contract-Obligations-by-Contract-Type-', Sys.Date(),'.csv', sep=''),
...
output$hover_info <- renderUI({
  ...
  hov_amount <- dataset() %>%
    filter(FY == year & Contract.Type == point$Contract.Type) %>%
    .$Amount %>%
    unlist
