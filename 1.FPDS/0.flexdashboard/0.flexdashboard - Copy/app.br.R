...
ui <- fluidPage(
  ...
  tags$style(HTML(".irs-single, .irs-to, .irs-from {background: #63c5b8}")),
  #tags$style(HTML(".irs-slider {background: black}")),
#  tags$style(HTML(".irs-grid-pol {display: absolute;}")),
  tags$style(HTML(".irs-max {background: #63c5b8; color: white}")),
  tags$style(HTML(".irs-min {background: #63c5b8; color: white}")),
  tags$style(HTML(".irs-bar-edge {border: 1px red}")),
  tags$style(HTML(".irs-bar-edge {border-color: 1px red}")),
  tags$style(HTML(".irs-bar-edge {border-color: 1px red}")),

  tags$style(HTML(".checkbox {background: #63c5b8} 
                  ")), 
  tags$style(HTML("
.checkbox .tag {
  color: #595959;
...
#                       size = 4),
           ...
           radioButtons("BreakoutType", "Breakout", 
                        c("All Contracts" = "All",
                          "Products by Category" = "Prd",
                          "Services by Category" = "Svc",
                          "R&D by Stage" = "RnD"),
    ...
    column(9, align = 'center',
           ...
           fluidRow(
             ...
             # on-click plot area
             column(12, align = 'center', plotOutput("clickplot", height = '255px')
             )
           )) 
...
server <- function(input, output, session){
  ...
  output$hover_info <- renderUI({
    ...
    style <- paste0(
      "position:fixed; z-index:100; background-color: rgba(245, 245, 245, 0.85); ",
      "left:", left_px + 2, "px; top:", top_px + 2, "px;")
    
    # figure out what to show on tooltip -LCL
    f_year <- levels(shown$FY)[round(hover$x)]
    total <- shown %>%
      filter(FY == f_year) %>%
      summarize(sum(Billion)) %>%
      unlist()
    
    if(hover$y > total) return()
    if(hover$y < 0) return()
    ...
    # i <- 1L
    # while(total - cat_values$Billion[i] > 0 & i <= length(cat_values$Category)){
    #   total <- total - cat_values$Billion[i]
    #   i <- i+1
    # }
    n <- 1L
    current <- cat_values$Billion[1]
    while(hover$y > current){
      n <- n+1
      current <- sum(cat_values$Billion[1:n])
      }
    cat_shown <- cat_values$Category[n]
    amount_shown <- round(cat_values$Billion[n],2)
    
    
    # actual tooltip created as wellPanel
    wellPanel(
      style = style,
      p(HTML(paste0("<b> Year: </b>", f_year, "<br/>",
